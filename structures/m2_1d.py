...
#dx = (10e-9, 10e-9, 1e-9)
dx = (1e-9, 1e-9, 1e-9)
n = (1,1,1)  #n = (round(L/dx[0]), round(L/dx[1]), round(1e-9/dx[2]))
origin=(0,0,0) #origin = (-x1, -y1, 0)
...
#    "A": 0,
    ...
    "p": state.Tensor((0, -1, 0)), #0, 0)), #No current in all structure, just horizontal stripe
    "d": d,
    "je": je}
...
#state.h = demag + exchange + torque + aniso
state.t = 0.
llg = LLGSolver([torque, aniso, external]) #demag, exchange, torque, aniso, external])
